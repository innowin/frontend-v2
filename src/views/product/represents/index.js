...
export class Post extends Component {
  ...
  _delete = () => {
	...
	getExchangeForPost = (post) => {
		socket.emit(REST_REQUEST,
			{
				method: "get",
				url: `${url}/exchanges/${post.post_parent}/`,
				result: `product-exchange-get/${post.post_parent}`,
				token: TOKEN
			}
		);

		socket.on(`product-exchange-get/${post.post_parent}`, (res) => {
			if (res.detail) {
  ...
  componentDidMount() {
    ...
    const emitting = () => {
      ...
      socket.emit(REST_REQUEST,
        {
          method: "get",
          url: `${url}/base/posts/post_user=${userId}&post_type=supply`,
          result: `product-Posts-get/${userId}`,
          token: TOKEN
        }
      );
      socket.emit(REST_REQUEST,
    ...
    socket.on(`product-Posts-get/${userId}`, (res) => {
      if (res.detail) {
        const newState = {...this.state, error: res.detail, isLoading: false};
        this.setState(newState);
      } else {
  ...
  componentWillUnmount() {
    ...
    socket.off(`product-Posts-get/${userId}`, (res) => {
      if (res.detail) {
        const newState = {...this.state, error: res.detail, isLoading: false};
        this.setState(newState);
      } else {
