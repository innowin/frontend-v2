...
class CarouselLogin extends Component {
  ...
  _renderCarouselSvg = (index) => {
    if (index === 0) {
      return <RocketSvg className='carousel-svg'/>
    }
    else if (index === 1) {
      return <PhoneSvg className='carousel-svg'/>
    }
    else if (index === 2) {
      return <BoxSvg className='carousel-svg'/>
    }
  }
  ...
  render() {
    ...
    return (
        <div className="carousel-wrapper">
          <TransitionGroup className='carousel-content'>
            {slides.map((slide, i) => (slide.id === activeId ?
                    // <---------------- slide
