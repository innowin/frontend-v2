...
export function* getFollowees(action) {
  const {followIdentity, followOwnerType, followOwnerId} = action.payload
  ...
  try {
    ...
    yield put({type: types.SUCCESS.COMMON.SOCIAL.GET_FOLLOWEES , payload:{data, followOwnerId, followIdentity, followOwnerType}})
  } catch (error) {
    const {message} = error
    yield put({
